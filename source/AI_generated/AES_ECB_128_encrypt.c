#include <stdio.h>
#include <stdint.h>
#include <string.h>

int main() {
    // Input plaintext (16 bytes)
    uint8_t input[16] = {
        'T', 'h', 'i', 's', ' ', 'i', 's', ' ', '1', '6', ' ', 'b', 'y', 't', 'e', 's'
    };

    // AES-128 key (16 bytes)
    uint8_t key[16] = {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
        0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
    };

    // S-box
    uint8_t sbox[256] = {
        
    };

    uint8_t rcon[11] = {
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    };

    uint8_t roundKeys[176];
    memcpy(roundKeys, key, 16);

    // Key Expansion
    int r = 1;
    for (int i = 16; i < 176; i += 4) {
        uint8_t temp[4];
        memcpy(temp, &roundKeys[i - 4], 4);

        if (i % 16 == 0) {
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            for (int j = 0; j < 4; j++)
                temp[j] = sbox[temp[j]];

            temp[0] ^= rcon[r++];
        }

        for (int j = 0; j < 4; j++)
            roundKeys[i + j] = roundKeys[i - 16 + j] ^ temp[j];
    }

    // Initial AddRoundKey
    uint8_t state[16];
    for (int i = 0; i < 16; i++)
        state[i] = input[i] ^ roundKeys[i];

    // AES Rounds
    for (int round = 1; round <= 10; round++) {
        // SubBytes
        for (int i = 0; i < 16; i++)
            state[i] = sbox[state[i]];

        // ShiftRows
        uint8_t temp[16];
        temp[0] = state[0];  temp[1] = state[5];  temp[2] = state[10]; temp[3] = state[15];
        temp[4] = state[4];  temp[5] = state[9];  temp[6] = state[14]; temp[7] = state[3];
        temp[8] = state[8];  temp[9] = state[13]; temp[10] = state[2]; temp[11] = state[7];
        temp[12] = state[12];temp[13] = state[1]; temp[14] = state[6]; temp[15] = state[11];
        memcpy(state, temp, 16);

        // MixColumns (skip in final round)
        if (round != 10) {
            for (int i = 0; i < 4; i++) {
                int idx = i * 4;
                uint8_t a = state[idx], b = state[idx+1], c = state[idx+2], d = state[idx+3];
                state[idx]   = (uint8_t)( (int)(a<<1 ^ ((a>>7)*0x1b)) ^ b ^ c ^ d );
                state[idx+1] = (uint8_t)( a ^ (int)(b<<1 ^ ((b>>7)*0x1b)) ^ c ^ d );
                state[idx+2] = (uint8_t)( a ^ b ^ (int)(c<<1 ^ ((c>>7)*0x1b)) ^ d );
                state[idx+3] = (uint8_t)( a ^ b ^ c ^ (int)(d<<1 ^ ((d>>7)*0x1b)) );
            }
        }

        // AddRoundKey
        for (int i = 0; i < 16; i++)
            state[i] ^= roundKeys[round * 16 + i];
    }

    // Output encrypted result
    printf("Encrypted output:\n");
    for (int i = 0; i < 16; i++)
        printf("%02x ", state[i]);
    printf("\n");

    return 0;
}