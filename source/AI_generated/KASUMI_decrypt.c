...
int main() {
    ...
    // Example 128-bit key and 64-bit ciphertext
    uint16_t key[8] = {0x0123,0x4567,0x89ab,0xcdef,0xfedc,0xba98,0x7654,0x3210};
    uint16_t ciphertext[4] = {0x0123,0x4567,0x89ab,0xcdef}; // Replace with actual ciphertext
    ...
    uint16_t L[4], R[4];
    memcpy(L, ciphertext, 4*sizeof(uint16_t));
    memcpy(R, ciphertext+2, 2*sizeof(uint16_t));
    ...
    plaintext[0] = L[0]; plaintext[1] = L[1];
    plaintext[2] = R[0]; plaintext[3] = R[1];

    printf("KASUMI plaintext: ");
    for(int i=0;i<4;i++) printf("%04x ", plaintext[i]);
    printf("\n");
    return 0;
}