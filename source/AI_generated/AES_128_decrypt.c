...
void InvMixColumns(uint8_t* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        c = state[4*i+2];
        d = state[4*i+3];
        state[4*i]   = xtime(xtime(xtime(a ^ c))) ^ xtime(xtime(b ^ d)) ^ a ^ b ^ c ^ d;
        state[4*i+1] = xtime(xtime(xtime(b ^ d))) ^ xtime(xtime(a ^ c)) ^ a ^ b ^ c ^ d;
        state[4*i+2] = xtime(xtime(xtime(a ^ c))) ^ xtime(xtime(b ^ d)) ^ a ^ b ^ c ^ d;
        state[4*i+3] = xtime(xtime(xtime(b ^ d))) ^ xtime(xtime(a ^ c)) ^ a ^ b ^ c ^ d;
    }
}
...
int main() {
    // Example 128-bit key and ciphertext
    uint8_t key[16] = {
        0x2b,0x7e,0x15,0x16,0x28,0xae,0xd2,0xa6,
        0xab,0xf7,0x09,0xcf,0x4f,0x3c,0x9e,0x11
    };
    ...
    AES128_Decrypt(ciphertext, key, plaintext);
    printf("Plaintext: ");
    for (int i = 0; i < 16; ++i)
