#include <stdio.h>
#include <stdint.h>
#include <string.h>

// S-box and Rcon as in previous files
static const uint8_t sbox[256] = {
    // ... S-box values ...
    
};
static const uint8_t Rcon[13] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36,0x6c,0xd8
};

void KeyExpansion(const uint8_t* key, uint8_t* roundKeys) {
    int i, j;
    uint8_t temp[4];
    memcpy(roundKeys, key, 24);
    for (i = 6; i < 52; ++i) {
        memcpy(temp, &roundKeys[4*(i-1)], 4);
        if (i % 6 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[i/6];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        }
        for (j = 0; j < 4; ++j)
            roundKeys[4*i+j] = roundKeys[4*(i-6)+j] ^ temp[j];
    }
}

void SubBytes(uint8_t* state) {
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

void ShiftRows(uint8_t* state) {
    uint8_t tmp;
    // Row 1
    tmp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = tmp;
    // Row 2
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    // Row 3
    tmp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = tmp;
}

uint8_t xtime(uint8_t x) {
    return ((x<<1) ^ (((x>>7) & 1) * 0x1b));
}

void MixColumns(uint8_t* state) {
    uint8_t Tmp, Tm, t;
    for (int i = 0; i < 4; ++i) {
        t = state[i*4];
        Tmp = state[i*4] ^ state[i*4+1] ^ state[i*4+2] ^ state[i*4+3];
        Tm = state[i*4] ^ state[i*4+1]; Tm = xtime(Tm); state[i*4] ^= Tm ^ Tmp;
        Tm = state[i*4+1] ^ state[i*4+2]; Tm = xtime(Tm); state[i*4+1] ^= Tm ^ Tmp;
        Tm = state[i*4+2] ^ state[i*4+3]; Tm = xtime(Tm); state[i*4+2] ^= Tm ^ Tmp;
        Tm = state[i*4+3] ^ t; Tm = xtime(Tm); state[i*4+3] ^= Tm ^ Tmp;
    }
}

void AddRoundKey(uint8_t* state, const uint8_t* roundKey) {
    for (int i = 0; i < 16; ++i)
        state[i] ^= roundKey[i];
}

void AES192_Encrypt(uint8_t* input, const uint8_t* key, uint8_t* output) {
    uint8_t state[16], roundKeys[208];
    memcpy(state, input, 16);
    KeyExpansion(key, roundKeys);
    AddRoundKey(state, roundKeys);
    for (int round = 1; round <= 11; ++round) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys + round*16);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys + 192);
    memcpy(output, state, 16);
}

int main() {
    // Example 192-bit key and plaintext
    uint8_t key[24] = {
        0x8e,0x73,0xb0,0xf7,0xda,0x0e,0x64,0x52,
        0xc8,0x10,0xf3,0x2b,0x80,0x90,0x79,0xe5,
        0x62,0xf8,0xea,0xd2,0x52,0x2c,0x6b,0x7b
    };
    uint8_t plaintext[16] = {
        0x6b,0xc1,0xbe,0xe2,0x2e,0x40,0x9f,0x96,
        0xe9,0x3d,0x7e,0x11,0x73,0x93,0x17,0x2a
    };
    uint8_t ciphertext[16];
    AES192_Encrypt(plaintext, key, ciphertext);
    printf("Ciphertext: ");
    for (int i = 0; i < 16; ++i)
        printf("%02x", ciphertext[i]);
    printf("\n");
    return 0;
}
